...
public class Counters {
   ...
   * @param <K>
   */
  public static <K> double makeProbsFromLogScoresInPlace(Counter<K> logScores)
  {
  ...
  public static <T> Counter<T> toCounter(double[] arr, List<T> indexer) {
    Counter<T> res = new Counter<T>();
    for (int i = 0; i < indexer.size(); i++) {
      T t = indexer.get(i);
      res.incrementCount(t, arr[i]);
    }
    return res;
  }
}
