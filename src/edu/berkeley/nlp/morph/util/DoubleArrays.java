...
public class DoubleArrays {
  ...
  public static void subtractInPlaceUnsafe(double[] x, double[] y) {
    ...
    public static double[] raiseToPower(double[] pUnexponentiated, double p) {
        double[] exponentiated = new double[pUnexponentiated.length];
        for (int index = 0; index < pUnexponentiated.length; index++) {
            exponentiated[index] = Math.pow(pUnexponentiated[index],p);
        }
        return exponentiated;
    }
  ...
  public static int sample(double[] arr, Random r) {
    double sum = DoubleArrays.add(arr);
    assert sum > 0.0;
  ...
  public static double addFrom(double[] arr, int start) {
  ...
  public static double addTill(double[] arr, int stop) {
    assert stop <= arr.length;
    double sum = 0.0;
    for (int i=0; i < stop; ++i) sum += arr[i];
    return sum;
  }
